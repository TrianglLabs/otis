...
/**
 * Below this width the panel would squeeze the transcript, so it stays hidden and traces stay
 * in the conversation.
 */
const SUBAGENT_PANEL_MIN_TERMINAL_WIDTH = 96
...
export class SubagentPanel {
  ...
  /**
   * Honors the `/settings subagents` preference. Width and empty-session rules still apply when
   * shown.
   */
  ...
  handleKey(key: UIKey) {
    ...
    if (key.name === "up" || key.name === "down") {
      ...
      // With a trace open, moving the focus swaps the open trace too.
      const id = this.#traces[this.#focusIndex]?.toolCallId
      if (this.#selectedId && id && id !== this.#selectedId) this.options.onSelect(id)
      return true
    }
  ...
  /**
   * Mounts the panel only while there is something to list and the terminal is wide enough for
   * two columns.
   */
  layout() {
    const shouldMount =
  ...
  private refresh() {
    this.options.footer.content = this.#selectedId
      ? FOOTER_OPEN
      : this.#focused
        ? FOOTER_FOCUSED
        : FOOTER_IDLE
  ...
  private syncFocusIndex(preferredId: string | undefined) {
    ...
    const index = preferredId
      ? this.#traces.findIndex((trace) => trace.toolCallId === preferredId)
      : -1
  ...
  private syncRows() {
    ...
    this.#traces.forEach((trace, index) => {
      ...
      const row = createPickerRow(this.options.renderer, subagentRowId(trace.toolCallId), {
        bg: "surface",
      })
  ...
  private paint(elapsedMs: number) {
    this.#traces.forEach((trace, index) => {
      const row = this.#rows.get(trace.toolCallId) as PickerRow
      const selected =
        trace.toolCallId === this.#selectedId ||
        (this.#focused && !this.#selectedId && index === this.#focusIndex)
      ...
      if (trace.status === "running")
        chunks.push(...shimmerText(title, elapsedMs, colors.text).chunks)
      else chunks.push(...t`${fg(selected ? colors.accent : colors.text)(title)}`.chunks)
      row.title.content = new StyledText(chunks)
