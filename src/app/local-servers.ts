...
import { discoverPairModels, normalizePairEndpoints, type PairEndpoints } from "../inference/pair.js"
import type { OmlxCatalogModel, PairCatalogModel } from "../inference/types.js"

export type LocalServerInputs = { ollama?: string; lmStudio?: string; omlx?: string; omlxApiKey?: string }
...
export async function prepareLocalServers(
  ...
  const requested = normalizePairEndpoints({ ollama: input.ollama?.trim(), lmStudio: input.lmStudio?.trim() })
  ...
  if (omlx) {
    const apiKey =
      input.omlxApiKey?.trim() || (previousOmlx?.baseURL === omlx.baseURL ? previousOmlx.apiKey : undefined)
  ...
  const [pairResult, omlxResult] = await Promise.allSettled([
    ...
    omlx ? (options.discoverOmlx ?? discoverOmlxModels)(omlx, { signal: options.signal }) : Promise.resolve(undefined),
  ])
  ...
  // An explicitly authenticated oMLX connection must never appear successful when its credentials fail.
  ...
  return {
    pairEndpoints: {
      ...
      ...(pair?.lmStudio !== undefined && requested.lmStudio ? { lmStudio: requested.lmStudio } : {}),
    },
