...
export class LlamaCppRuntime {
  ...
  async #killProcess(child = this.#process) {
    ...
    child.kill("SIGKILL")
    deadline = Date.now() + KILL_WAIT_MS
    while (!processHasTerminated(child) && Date.now() < deadline) await sleep(50)
  }
  ...
  async #start(
    ...
    try {
      ...
      signal.throwIfAborted()
      // Retry on Vulkan only for diagnostics identifying the CUDA backend. A generic model,
      ...
      if (!runtime.managed || runtime.hardware.backend !== "cuda" || !cudaFailure) throw error
      const vulkan = { ...runtime.hardware, backend: "vulkan" as const, cudaVersion: undefined }
      const fallback = await this.#resolveBinary(model, vulkan, signal, onProgress)
      signal.throwIfAborted()
  ...
  async #resolveBinary(
    ...
    const bundleName =
      hardware.backend === "cuda" ? `${releaseTag}-cuda-${hardware.cudaVersion}` : releaseTag
    ...
    if (hardware.backend === "vulkan") {
      throw new Error(
        `Vulkan GPU acceleration is unavailable: ${failure}. Check the GPU driver and device access.`,
      )
    }
    const vulkan = { ...hardware, backend: "vulkan" as const, cudaVersion: undefined }
    return await this.#resolveBinary(model, vulkan, signal, onProgress)
  }
