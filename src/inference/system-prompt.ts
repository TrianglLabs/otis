...
export function buildSystemPrompt(
  ...
  if (projectContext.length > 0) {
    let totalBytes = 0
    const files = projectContext.map((file, index) => {
      ...
      return `<file path="${escapeText(path).replaceAll('"', "&quot;")}">\n${content}\n</file>`
    })
    sections.push(
  ...
  if (skills.length > 0) {
    const entries = skills.map(
      (skill) =>
        `  <skill name="${escapeText(skill.name).replaceAll('"', "&quot;")}">${escapeText(skill.description)}</skill>`,
    )
    sections.push(
