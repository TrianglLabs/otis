...
import type { ChatMessage, CompleteOptions, InferenceClient, LocalClientConfig, StreamChatOptions } from "./types.js"

export class LlamaCppClient implements InferenceClient {
  ...
  constructor(config: LocalClientConfig) {
    this.model = requiredText(config.model, "Local model")
    this.#fetch = config.fetch ?? fetch
    this.#inferenceURL = inferenceEndpointURL(config.inferenceURL, "Local inference URL")
  ...
  async *streamChat(options: StreamChatOptions) {
    ...
    if (!response.ok) {
      throw new Error(`Local model request failed with HTTP ${response.status}: ${await responsePreview(response)}`)
    }
    if (!response.body) throw new Error("Local model response did not include a stream body")
