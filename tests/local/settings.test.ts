...
describe("local settings", () => {
  ...
  it("migrates the original theme names to the renamed themes", async () => {
    ...
    for (const [oldName, newName] of [
      ...
      const file = join(directory, `${oldName}.json`)
      await writeFile(file, JSON.stringify({ version: 1, theme: oldName }), "utf8")
      await expect(loadLocalSettings({ file, env: {} })).resolves.toMatchObject({ theme: newName })
    }
  })
